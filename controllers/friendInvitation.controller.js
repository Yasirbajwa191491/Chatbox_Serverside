...
const inviteFriend = async (req, res) => {
   ...
   const invite= await FriendInvitation.create({
    ...
    return res.status(201).send({message:"Invitation has been sent successfully",data:invite});
};
...
const acceptInvitation = async (req, res) => {
    ...
    try {
        ...
        const receiver = await User.findById(deletedInvitation.receiverId);
        ...
        updateUsersInvitations(deletedInvitation.receiverId.toString());

        // update the user's(user accepting the invitation, receiver) friends list
        updateUsersFriendsList(deletedInvitation.receiverId.toString());
        ...
        return res.status(500).send("Sorry, something went wrong. Please try again later",err);
    }
};
