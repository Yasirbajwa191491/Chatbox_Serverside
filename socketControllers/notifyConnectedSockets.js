...
const sendNewDirectMessage = async (conversationId, newMessage,senderId,receiverUserId) => {
    ...
    if (conversation) {
        const notificationIndex = conversation?.notifications.findIndex(
            (notification) => notification.userId.toString() === senderId.toString()
          );
          console.log(newMessage._id,'notificationIndex');
          conversation?.notifications[notificationIndex]?.allmessages?.push(newMessage._id);
        // conversation.messages = [...conversation.messages, newMessage._id];
       let newConversation= await conversation.save();
       if(newConversation.notifications.length===0){
        newConversation.notifications.push({
            userId:senderId,
...
const sendNewGroupMessage = async (groupChatId, newMessage) => {
    ...
    let remainingParticipant=groupChat?.participants?.filter((curEle)=>{
        return curEle.toString() !==newMessage.author.toString();
    })
    remainingParticipant?.map((userId)=>{
        GroupChat.findOne({
            _id: groupChatId,
            ...
            .then((groupChat) => {
              if (groupChat) {
                GroupChat.findOneAndUpdate(
                    { _id: groupChatId, 'notifications.userId': userId },
                    {
                      $push: {
                        'notifications.$.allmessages': newMessage._id,
                      },
                    },
                    { new: true }
                  )
                    .exec()
                    .then(() => {
                      console.log("Message ID added to allmessages within the specified GroupChat");
                    })
              } else {
                ...
                GroupChat.findOneAndUpdate(
                    ...
                    .exec()
                    .then(() => {
                      console.log("User IDs and message ID added to GroupChat");
                    })
              }
            })
            .catch((error) => {
              console.error("Error checking userIds in notifications:", error);
            });
    })
